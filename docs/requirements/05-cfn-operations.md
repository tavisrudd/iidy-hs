...
Operations are implemented as pure functions over a `CfnContext` and an output emitter
`(OutputData -> IO ())`. This design isolates AWS I/O from output formatting and makes
each operation independently testable. The `OutputDispatch` layer routes emitted
`OutputData` to the active renderer (interactive or JSON) based on the user's CLI flags.

## Implementation Context

- Primary module directory: `src/Iidy/Cfn/Operations/`
- Shared infrastructure: `src/Iidy/Cfn/StackOperations.hs` (polling, event fetch,
  content collection), `src/Iidy/Cfn/Context.hs` (CfnContext), `src/Iidy/Confirm.hs`
  (confirmation prompts)
- Output types: `src/Iidy/Output/Types.hs` (all OutputData constructors)
- Request building: `src/Iidy/Cfn/RequestBuilder.hs`
...
timing text every 1 second). The output emitter signature is uniform across all
operations: `(OutputData -> IO ())`.
...
```
buildCreateStackRequest (primary token)
  → Amazonka.send CreateStack
  → getStack → emit OdStackDefinition
  → emit OdPollingStarted
  → pollForCompletion (allTerminalStatuses, 2s interval)
      pcOnNewEvents:        emit OdNewStackEvents
      pcOnOperationComplete: emit OdOperationComplete
  → if DELETE_COMPLETE: return Right 1
  → collectStackContents → emit OdStackContents
  → if CREATE_COMPLETE: return Right 0 else return Right 1
```

**Edge Cases:**

- Stack ID is extracted from the `CreateStackResponse`; if absent, falls back to
  `stackName` for polling.
- `getStack` may return `Nothing` immediately after create (race); `OdStackDefinition`
  emission is guarded by `case mStack of Just ... Nothing -> pure ()`.
- The primary client request token is used (not a derived token) to allow safe retries
  of the create call.

**Error Scenarios:**

- Template load failure: return `Left` error before any AWS call.
- CloudFormation API error: return `Left (show awsErr)` or re-throw, depending on error
  type.
- `ROLLBACK_COMPLETE` terminal status: return exit code 1 (create failed, stack
  remains).
- `DELETE_COMPLETE`: return exit code 1 (create failed, stack cleaned up).

**Complexity Notes:**

The primary token is passed as `usePrimary = True` to `buildCreateStackRequest`; this
enables idempotent retries on network failure. S3 upload logic is centralized in
`RequestBuilder`, not in this operation.
...
```
buildUpdateStackRequest (primary token)
  → try (Amazonka.send UpdateStack)
  → Left isNoUpdatesError:
      getStack → emit OdStackDefinition
      throwIO awsErr          -- top-level handler displays it; exit 1
  → Left other: return Left (show awsErr)
  → Right resp:
      getStack → emit OdStackDefinition
      emit OdPollingStarted
      pollForCompletion (allTerminalStatuses, 2s)
          pcOnNewEvents:        emit OdNewStackEvents
          pcOnOperationComplete: emit OdOperationComplete
      collectStackContents → emit OdStackContents
      if UPDATE_COMPLETE: Right 0 else Right 1
```
...
- `isNoUpdatesError` checks `Amazonka.ServiceError` message for the substring
...
The `try`/`catch` pattern is applied only to the `UpdateStack` API call, not to the
polling phase. This keeps the "no updates" check focused without swallowing other
errors.
...
```
getStack → emit OdStackDefinition
csName = "iidy-update-" <> take 8 (tiValue primaryToken)
createChangeset (UPDATE type, csName)
  → emit OdChangeSetResult
  → if csiStatus == "FAILED": return Left statusReason
  → confirmChangesetExecution yesFlag
      → if declined: return Right 130
...
- Changeset `FAILED`: `Left (statusReason)`, no execution.
- User declines: exit code 130.
- Execution fails: exit code from `executeChangeset`.

**Complexity Notes:**

Changeset name generation for update-stack uses the primary token prefix directly.
Create-or-update uses `"iidy-create-or-update-" <> take 8 primaryToken`. These are
distinct to avoid naming conflicts when both paths are used on the same token.
...
getStack
  → Nothing:
      getCallerIdentity → emit OdStackAbsentInfo → return Right 0
  → Just cfnStack:
      emit OdStackDefinition
      fetchStackEvents → emit OdStackEvents (max 10, with durations)
      collectStackContents → emit OdStackContents
      requestConfirmation (unless skipConfirmation)
          → declined: return Right 130
      getStackId → pollTarget
      buildDeleteStackRequest → Amazonka.send DeleteStack
      emit OdPollingStarted
      pollForCompletion (pollTarget, allTerminalStatuses, 2s)
          pcOnNewEvents:        emit OdNewStackEvents
          pcOnOperationComplete: emit OdOperationComplete
      if DELETE_COMPLETE: Right 0 else Right 1
```
...
- If `getStackId` returns `Nothing`, falls back to polling by stack name (best effort).
...
The confirmation prompt is implemented in `Iidy.Confirm.requestConfirmation`. It
performs TTY detection on `stdout` and uses `LineBuffering` on `stdin` for reliable
line reads. The ANSI escape sequence for bold bright red is `\ESC[1;91m`, reset is
`\ESC[0m`.
...
- `ROLLBACK_COMPLETE` status is treated as absent (stack exists but is broken; treat as
  absent for create-or-update purposes). This is handled by `stackExists` returning
  `False` for `DELETE_COMPLETE`, and by the broader definition of "stack absent" in
  `checkStackState`.
...
```
generateDashedName → csName (random adjective-noun, e.g. "swift-tiger")
createChangeset (CREATE type, csName, stackExists=False)
  → getStack → emit OdStackDefinition (stack now in REVIEW_IN_PROGRESS)
  → buildChangeSetCreationResult → emit OdChangeSetResult
  → if FAILED: return Left statusReason
  → confirmChangesetExecution yesFlag
      → declined: return Right 130
  → executeChangeset ctx stackName csName emit
```
...
createChangeset (UPDATE type, csName, stackExists=True)
  → emit OdChangeSetResult
  → if FAILED: return Left statusReason
  → confirmChangesetExecution yesFlag
      → declined: return Right 130
...
- Random changeset names for CREATE path use `generateDashedName` (adjective-noun
  vocabulary): `["red","blue","green","happy","clever","brave","swift","mighty"]` x
  `["cat","dog","bird","fish","lion","eagle","shark","tiger"]`. These are used only when
  the user has not provided a changeset name.
- For the CREATE changeset path: after `createChangeset` completes, the stack exists in
  `REVIEW_IN_PROGRESS`; `getStack` is called to fetch and emit its definition.

**Error Scenarios:**

- Stack existence check failure: AWS error propagated as exception.
- Any path that returns `Left`: error displayed by top-level handler.
- User declines any confirmation: exit code 130.

**Complexity Notes:**

The `stackExists` function returns `False` for `DELETE_COMPLETE` stacks. However,
`getStack` still returns `Just` for such stacks (they are queryable). The distinction
matters: `stackExists` is used for routing; `getStack` is used when fetching the
...
```
ctxDeriveToken "execute-changeset"
  → Amazonka.send ExecuteChangeSet
  → getStackId → stackId (fallback to stackName)
  → getStack → emit OdStackDefinition
  → fetchStackEvents → emit OdStackEvents (max 10, title "Previous Stack Events (max 10):")
  → emit OdPollingStarted
  → pollForCompletion (stackId, allTerminalStatuses, 2s)
      pcOnNewEvents:        emit OdNewStackEvents
      pcOnOperationComplete: emit OdOperationComplete
  → if not DELETE_COMPLETE: collectStackContents → emit OdStackContents
  → if CREATE_COMPLETE or UPDATE_COMPLETE: Right 0 else Right 1
```
...
- Transient errors during changeset polling (`describeChangesetRaw` returning `Left`)
  are ignored; polling continues.

**Error Scenarios:**

- Changeset `FAILED` after creation: `Left statusReason`.
- `ExecuteChangeSet` API error: propagated as exception.
- `DELETE_COMPLETE` after execution: exit code 1.

**Complexity Notes:**

`buildChangeSetCreationResult` constructs the `ChangeSetCreationResult` from a
`ChangeSetInfo`. The `csrNextSteps` field contains ready-to-paste CLI commands. The
`csrPendingChangesets` field holds the full `ChangeSetInfo` list for the JSON renderer
to emit.
...
getStack
  → Nothing:
      getCallerIdentity → emit OdStackAbsentInfo → return Right ()
  → Just cfnStack:
      fetchStackEvents
      collectStackContents
      convertStack cfnStack regionText → stackDef
      buildEventsDisplay stackName numEvents events → eventsDisplay
      emit OdStackDefinition
      emit OdStackEvents
...
- Events are returned most-recent-first from AWS; `buildEventsDisplay` takes the first
  `numEvents` and then passes them to `calculateEventDurations` which sorts
...
- AWS API error in `collectStackContents`: propagated as exception.

**Complexity Notes:**

`describe-stack` and `delete-stack` (pre-delete display) share the
`buildEventsDisplay`/`calculateEventDurations` pair for historical events. Live polling
events use `convertEventWithDuration` (duration from operation start time) instead.
...
```
getStack
  → Nothing: return Left "Stack not found: <name>"
  → Just cfnStack:
      emit OdStackDefinition
      getStackId → sId
      fetchStackEvents sId → initialEvents
      buildEventsDisplay stackName 10 initialEvents → emit OdStackEvents
      seenIds = map (.eventId) initialEvents
      emit OdPollingStarted
      pollForCompletion (sId, allTerminalStatuses, pcWaitForStatusChange=True,
                         pcInactivityTimeoutSecs=timeoutSeconds)
          pcOnNewEvents: filter out seenIds → emit OdNewStackEvents
          pcOnOperationComplete: emit OdOperationComplete
          pcOnInactivityTimeout: emit OdInactivityTimeout
      → finalStatus
      if DELETE_COMPLETE: return Right 0
      else: collectStackContents → emit OdStackContents → return Right 0
```

**Edge Cases:**

- `pcWaitForStatusChange = True` ensures watch-stack does not exit instantly on a stack
  that is already in `UPDATE_COMPLETE` from a prior operation. Polling only terminates
  on a terminal status after at least one new event has been emitted.
- The inactivity timeout only fires after new events have been seen (controlled by the
  `hasSeenNewEventsRef` in `pollForCompletionWith`).
...
- Stack not found: `Left` error, non-zero exit (unlike other absent-stack operations
  which return exit 0).
- Inactivity timeout fires: `OdInactivityTimeout` emitted, then `Right 0`.
- Overall timeout fires: return `Right 0` with empty final status.

**Complexity Notes:**

watch-stack is the only operation that sets `pcWaitForStatusChange = True` in its
`PollConfig`. This flag changes the terminal-status check in `pollForCompletionWith`
from "check immediately" to "check only after `hasSeenNewEventsRef` is True".
...
getStack
  ...
  → Just cfnStack:
      emit OdStackDefinition
      needsDriftCheck cfnStack driftCacheSecs
        → if True:
            getCurrentTime → emit OdStatusUpdate "Checking for stack drift..."
            Amazonka.send DetectStackDrift → detectionId
            pollDriftDetection ctx detectionId (3s interval)
      collectDriftData ctx stackName
        → fetchAllDriftPages (paginated DescribeStackResourceDrifts)
        → filter (not . isInSync)
        → map convertDrift
      emit OdStackDrift
      return Right ()
...
- `IN_SYNC` resources are filtered out before building `OdStackDrift`; the output
  contains only drifted resources.
- Pagination: `DescribeStackResourceDrifts` is paginated; `fetchAllDriftPages` recurses
  on `nextToken` until exhausted.

**Error Scenarios:**

- Stack not found: `Left` error.
- Drift detection API error: propagated as exception.
- All resources are `IN_SYNC`: `OdStackDrift` emitted with empty `sdrDriftedResources`
  list.

**Complexity Notes:**

`pollDriftDetection` uses a recursive loop with 3-second delay, distinct from the main
`pollForCompletion` infrastructure (which handles stack events). Drift detection polling
is simpler: no event callbacks, no timeout, just wait for non-`DETECTION_IN_PROGRESS`.
...
- All write operations (create-stack, update-stack, delete-stack, exec-changeset)
  use `pollForCompletion` with a 2-second interval.
- `pollForCompletion` is a thin wrapper over `pollForCompletionWith` which takes an
  explicit event-fetching function (enabling unit testing without AWS).
- `PollConfig` fields:
  - `pcIntervalSeconds`: 2 (default).
  - `pcTimeoutSeconds`: optional overall timeout; empty string status returned on
    expiry.
  - `pcInactivityTimeoutSecs`: optional inactivity timeout; only triggers after at least
    one new event has been seen (unless `pcWaitForStatusChange` is False).
  - `pcWaitForStatusChange`: False for all operations except watch-stack.
  - `pcOnNewEvents`: callback receiving reversed (chronological) new event list.
  - `pcOnOperationComplete`: callback receiving `OperationCompleteInfo` (elapsed
    seconds, start time, `skipRemainingSections` flag).
  - `pcOnInactivityTimeout`: callback receiving `InactivityTimeoutInfo`.
  - `pcOnPollTick`: called each poll cycle (used by spinner infrastructure).
- `OdPollingStarted` is emitted before `pollForCompletion` is called in every write
  operation.
...
**Logic Flow (pollForCompletionWith inner loop):**

```
startTime ← getCurrentTime (or pcStartTime)
lastEventTimeRef ← IORef startTime
hasSeenNewEventsRef ← IORef False
loop:
  threadDelay (pcIntervalSeconds * 1_000_000)
  pcOnPollTick
  events ← fetchEvents
  now ← getCurrentTime
  newEvents = filter (eventId `notElem` lastEventIds) events
  if not (null newEvents):
    writeIORef lastEventTimeRef now
    writeIORef hasSeenNewEventsRef True
    pcOnNewEvents (reverse newEvents)  -- chronological order
  check inactivity timeout:
    if configured && null newEvents && inactivityElapsed > timeout
       && (not pcWaitForStatusChange || hasSeenNewEvents):
      pcOnInactivityTimeout ...
      return ""
  check overall timeout → return ""
  check terminal status:
    if shouldCheckTerminal && currentStatus ∈ terminalStatuses:
      pcOnOperationComplete ...
      return currentStatus
  recurse with lastEventIds = map (.eventId) events
```

**Edge Cases:**

- New events are delivered to `pcOnNewEvents` in chronological order (reversed from AWS
...
- Duration calculation for live events uses `convertEventWithDuration`: `max(1, floor(eventTime - operationStartTime))` seconds.
...
`pollForCompletionWith` is exported and takes an `IO [CF.StackEvent]` action, enabling
pure unit tests via mock event sequences. This is the dependency-injection seam for the
polling system. The real implementation `pollForCompletion` simply calls
`pollForCompletionWith (fetchStackEvents ctx sId) sId`.
...
- All unrecognized AWS errors: propagated as exceptions to the top-level handler in
  `Main.hs`.
...
**Logic Flow (isNoUpdatesError):**

```
isNoUpdatesError (Amazonka.ServiceError se) =
  case se.message of
    Just msg → "No updates are to be performed" `isInfixOf` fromErrorMessage msg
    Nothing  → False
...
- `getStack` catches AWS `ValidationError` messages containing `"does not exist"` and
  returns `Nothing`; other errors are re-thrown.
- `stackExists` returns `False` for `DELETE_COMPLETE` stacks (not just absent stacks).
- Event duration minimum is 1 second: `max 1 (floor (diffUTCTime end start))`.

**Error Scenarios:**

- Lint: template > 51200 bytes emits a warning and skips validation; no error exit.
- Estimate-cost: template load failure returns `Left` error.
- Describe-stack-drift: stack not found returns `Left`; drift detection errors
  propagated as exceptions.

**Complexity Notes:**

The `StackAbsentInfo` type carries full STS identity (account, auth ARN) so the
renderer can show whether the user is authenticated to the expected account and region.
This directly addresses a common failure mode: running commands against the wrong
account.
...
- The primary token (`cfnPrimaryToken`) is used directly for single-step operations
  (CreateStack, UpdateStack with `usePrimary = True`).
- Multi-step operations derive per-step tokens via `ctxDeriveToken`:
  - `deriveTokenForStep primaryToken stepName` produces a deterministic token by
    hashing `(tiValue primaryToken) <> ":" <> stepName` with SHA256, then taking the
    first 36 characters.
  - `exec-changeset` derives token with step name `"execute-changeset"`.
- Derived tokens are tracked in `cfnUsedTokens` (IORef) for the command metadata
  summary.
- Changeset names are deterministic from the token prefix:
  - update-stack path: `"iidy-update-" <> take 8 primaryToken`
  - create-or-update update path: `"iidy-create-or-update-" <> take 8 primaryToken`
  - create-or-update create path: `generateDashedName` (random adjective-noun)
- `cfnPrimaryToken.tiValue` is a UUID string; `take 8` produces the first 8 hex
  characters of the UUID.

**Logic Flow:**

```
-- Primary token creation (startup)
primaryToken = user-provided --client-request-token
            | generateUUID

-- Step token derivation
deriveTokenForStep primaryToken step =
  TokenInfo { tiValue = take 36 (sha256hex (tiValue primaryToken <> ":" <> step))
            , tiSource = Derived }
...
- The create-or-update CREATE path uses a random name (`generateDashedName`) rather than
  a deterministic one, because at the point of creation the stack does not yet exist and
  there is no stable name to derive from.

**Error Scenarios:**

- Token too long for CloudFormation (max 128 characters): `buildCreateStackRequest`
  truncates to the API limit.
- Duplicate changeset name: CloudFormation API error propagated to caller.

**Complexity Notes:**

`cfnUsedTokens` is an `IORef [TokenInfo]` initialized with the primary token. The
`CommandMetadata` output includes all used tokens so the operator can reference them for
CloudFormation console lookups and retries. `ctxGetUsedTokens` returns them in
forward-chronological order (the IORef prepends; the accessor reverses).

---

## Testing Requirements

- All polling logic is tested via `pollForCompletionWith` with mock event sequences
  (no AWS calls required). Test scenarios include: immediate terminal status, status
  change after N iterations, inactivity timeout, `pcWaitForStatusChange` behavior.
- `isNoUpdatesError` is unit-tested with exact error message strings.
- `isConfirmation` is unit-tested covering `"y"`, `"yes"`, `"Y"`, `"YES"`, `""`,
  `"no"`, `"n"`, and arbitrary strings.
- `calculateEventDurations` is unit-tested with chronological and reverse-chronological
  event sequences, paired and unpaired events, and minimum-duration enforcement.
- `convertEventWithDuration` is unit-tested with known timestamps to verify `max 1`
  clamping and floor behavior.
- `buildEventsDisplay` is unit-tested for truncation info and title format.
- `checkStackState` and `generateDashedName` are unit-tested.
- `buildChangeSetCreationResult` is unit-tested for console URL format (percent-encoding
  of ARNs), `csrNextSteps` text format, `csrChangesetType` values.
- `buildConsoleUrl` is unit-tested for stack info URL format (no percent-encoding).
- `extractRegionFromArn` is unit-tested with standard and malformed ARNs.
...
- All AWS calls are mocked via the `CfnContext.cfnEnv` dependency; no real AWS
  credentials are required in the test suite.
- 100% of tests must pass before any commit. No test stubs, no skipped assertions.

## Cross-References

- `src/Iidy/Cfn/StackOperations.hs` — `pollForCompletion`, `pollForCompletionWith`,
  `PollConfig`, `defaultPollConfig`, `getStack`, `getStackId`, `stackExists`,
  `fetchStackEvents`, `collectStackContents`
- `src/Iidy/Cfn/Context.hs` — `CfnContext`, `createContext`, `ctxDeriveToken`,
  `createSuccessStates`, `updateSuccessStates`, `deleteSuccessStates`
- `src/Iidy/Confirm.hs` — `requestConfirmation`, `isConfirmation`
- `src/Iidy/Cfn/RequestBuilder.hs` — `buildCreateStackRequest`,
  `buildUpdateStackRequest`, `buildDeleteStackRequest`, `buildCreateChangeSetRequest`
- `src/Iidy/Cfn/TemplateLoader.hs` — `loadCfnTemplate`, `templateMaxBytes` (51200)
- `src/Iidy/Output/Types.hs` — all `OutputData` constructors referenced above
- `src/Iidy/Cfn/CommandMetadata.hs` — `constructCommandMetadata`,
  `createFinalCommandSummary` (emitted at end of all write operations)
- `src/Iidy/Aws/Sts.hs` — `getCallerIdentity` (used in absent-stack paths)
- `src/Iidy/Aws/ClientReqToken.hs` — `TokenInfo`, `deriveTokenForStep`
- Rust reference: `~/src/iidy/src/cfn/` (read-only)
- Phase research notes: `notes/phases/phase-13-research/`
