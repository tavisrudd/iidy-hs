...
import Iidy.Yaml.Handlebars.Helpers (defaultHelpers, HelperFn)
...
parseBlockOpen input = do
  ...
  (expr, rest) <- if T.isPrefixOf "}}" afterName'
    ...
    case val of
      Array arr -> do
        texts <- V.iforM arr $ \i item -> do
          let itemCtx = mergeContext ctx $ Object $ KM.fromList
                [ ("this", item)
...
lookupPath (seg : rest) val = case val of
  Object obj -> case KM.lookup (Key.fromText seg) obj of
    ...
    Nothing -> Null
  Array arr
    | Just i <- readInt seg, i >= 0, i < V.length arr ->
