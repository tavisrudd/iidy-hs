...
import Data.YAML.Event (ScalarStyle (..), Tag, tagToText)
...
parseYaml input uri =
    let
        -- Ensure trailing newline for HsYAML compatibility
        input' =
            if BL.null input || BL.last input /= 0x0a
                then input <> "\n"
                else input
     in
...
yamlLoader uri =
    Y.Loader
        { Y.yScalar = \tag style text pos ->
            pure $
                resolveScalar uri pos tag style text
        , Y.ySequence = \tag items pos ->
            pure $
...
makeScalarMeta uri pos tag text =
    let startP = convertPos pos
        ...
        endP =
            startP
                { posColumn = posColumn startP + tagLen + textLen
    ...
    AstMapping pairs _ ->
        case (getField "test" pairs, getField "then" pairs) of
            (Just test, Just thenVal) -> do
                ...
                pure $
                    AstPreprocessingTag
                        (PpIf (IfTag test thenVal (getField "else" pairs)))
                        meta
    ...
    AstMapping pairs _ ->
        case (getField "items" pairs, getField "template" pairs) of
            (Just items, Just templ) -> do
                ...
                pure $
                    AstPreprocessingTag
                        (mk items templ (getTextField "var" pairs) (getField "filter" pairs))
                        meta
    ...
    AstMapping pairs _ ->
        case (getField "items" pairs, getField "template" pairs) of
            (Just items, Just templ) -> do
                ...
                pure $
                    AstPreprocessingTag
                        (PpMergeMap (MergeMapTag items templ (getTextField "var" pairs)))
                        meta
    ...
    AstMapping pairs _ ->
        case (getField "items" pairs, getField "template" pairs) of
            (Just items, Just templ) -> do
                ...
                pure $
                    AstPreprocessingTag
                        (PpMapValues (MapValuesTag items templ (getTextField "var" pairs)))
                        meta
    ...
    AstMapping pairs _ ->
        case (getField "items" pairs, getField "key" pairs) of
            (Just items, Just key) -> do
                ...
                pure $
                    AstPreprocessingTag
                        (PpGroupBy (GroupByTag items key (getTextField "var" pairs) (getField "template" pairs)))
                        meta
