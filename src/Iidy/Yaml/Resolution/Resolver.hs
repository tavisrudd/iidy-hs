...
import Iidy.Yaml.CustomResources.Expansion (ExpansionResult (..), expandCustomResource)
import Iidy.Yaml.Emitter (emitYaml)
import Iidy.Yaml.Handlebars.Engine (InterpolateError (..), defaultHelpers, interpolate)
...
data ResolveErrorKind
    = -- | path, available vars
      REVariableNotFound !Text ![Text]
    | -- | expr, detail, varPath
      REJmesPath !Text !Text !Text
    | -- | missingKey, varPath, availKeys
      REPropertyNotFound !Text !Text ![Text]
    | -- | expected, found, contextTag
      RETypeMismatch !Text !Text !(Maybe Text)
    | -- | cfnTagName
      RECfnValidation !Text
    | -- | handlebars template error
      REHandlebars
    | -- | tagName (if extractable)
      RETagSyntax !(Maybe Text)
    | -- | templateName
      REExpandNotFound !Text
    | -- | templateName already being expanded
      RECircularExpansion !Text
    | -- | !$parseYaml/Json/expand parse error
      REParseSyntax
    | -- | fallback
      REGeneric
...
variableNotFoundError meta path available =
  ...
  where
    msg = "Variable not found: " <> path <> ". Available: " <> T.intercalate ", " available
...
jmesPathError meta expr detail varPath =
  ...
  where
    msg = "Invalid JMESPath expression '" <> expr <> "': " <> detail <> ". Variable: " <> varPath
...
propertyNotFoundError meta key varPath availKeys =
  ...
  where
    msg = "property '" <> key <> "' not found in mapping. Variable: " <> varPath <> ". Keys: " <> T.intercalate ", " availKeys
...
expandNotFoundError meta templateName =
  ...
  where
    msg = "!$expand: template '" <> templateName <> "' not found"
...
circularExpansionError meta templateName =
  ...
  where
    msg = "Circular template expansion detected: '" <> templateName <> "' is already being expanded"
      ...
      where
        expandOne parentNames (acc, globals) (resName, resVal) =
            case getResourceType resVal of
                Just typeName
                    | Just tmplInfo <- Map.lookup typeName (tcCustomTemplateDefs ctx) ->
    ...
    go t = case T.breakOn "{{" t of
        ...
        (_, rest) ->
            let inside = T.drop 2 rest
             in if T.isPrefixOf "#" inside
                    || T.isPrefixOf "/" inside
                    || T.isPrefixOf "!" inside
                    || T.isPrefixOf "else}}" inside
...
validateCfnTag meta name val = case name of
    "!Ref" -> case val of
        ...
        OString _ -> pure ()
        _nonString -> cfnValidationError meta "!Ref" $ "!Ref expects a string (resource or parameter name), found " <> cfnTypeName val
    "!Sub" -> case val of
        ...
        _nonStringOrArray -> cfnValidationError meta "!Sub" $ "!Sub expects a string or 2-element array, found " <> cfnTypeName val
    "!GetAtt" -> case val of
        ...
        _nonStringOrArray -> cfnValidationError meta "!GetAtt" $ "!GetAtt expects a string or 2-element array, found " <> cfnTypeName val
    "!Join" -> case val of
        ...
        _nonArray -> cfnValidationError meta "!Join" $ "!Join expects a 2-element array, found " <> cfnTypeName val
    "!Select" -> case val of
        ...
        _nonArray -> cfnValidationError meta "!Select" $ "!Select expects a 2-element array, found " <> cfnTypeName val
    "!Split" -> case val of
        ...
        _nonArray -> cfnValidationError meta "!Split" $ "!Split expects a 2-element array, found " <> cfnTypeName val
    "!FindInMap" -> case val of
        ...
        _nonArray -> cfnValidationError meta "!FindInMap" $ "!FindInMap expects a 3-element array, found " <> cfnTypeName val
    "!If" -> case val of
        ...
        _nonArray -> cfnValidationError meta "!If" $ "!If expects a 3-element array, found " <> cfnTypeName val
    "!Equals" -> case val of
        ...
        _nonArray -> cfnValidationError meta "!Equals" $ "!Equals expects a 2-element array, found " <> cfnTypeName val
    "!Not" -> case val of
        ...
        _nonArray -> cfnValidationError meta "!Not" $ "!Not expects a 1-element array, found " <> cfnTypeName val
    -- Null-only validation for remaining tags (matches Rust catch-all)
    _otherTag -> case val of
...
applyDotQueryValidated meta varPath q val
    ...
    | T.isInfixOf "," q = case val of
        ...
        _nonObject -> typeMismatchError meta "mapping" val
        ...
        OArray arr -> do
            results <-
                imapMaybeM
                    ...
                    arr
            pure $ OArray results
        _nonArray -> typeMismatchError meta "sequence" itemsVal
...
resolveMerge ctx meta (MergeTag sources) = do
    ...
    let merge acc v = case v of
            ...
            _nonObject -> typeMismatchError meta "object" v
    foldM merge (OObject []) vals

mergeOObjects :: OValue -> [(Text, OValue)] -> OValue
mergeOObjects (OObject base) overlay =
    let
        -- Update base values where overlay has same key
        updatedBase =
            map
                ( \(k, v) -> case lookup k overlay of
                    Just v' -> (k, v')
                    Nothing -> (k, v)
                )
                base
...
resolveConcatMap ctx meta (ConcatMapTag items template var filterExpr) = do
    ...
    case result of
        ...
        _nonArray -> tagSyntaxError meta (Just "!$concatMap") "!$concatMap: unexpected result"
  where
...
resolveMergeMap ctx meta (MergeMapTag items template var) = do
    ...
    case result of
        ...
        _nonArray -> tagSyntaxError meta (Just "!$mergeMap") "!$mergeMap: unexpected result"
...
resolveMapListToHash ctx meta (MapListToHashTag items template var filterExpr) = do
    ...
    case result of
        OArray arr -> do
            ...
            pure $ OObject pairs
        _nonArray -> tagSyntaxError meta (Just "!$mapListToHash") "!$mapListToHash: unexpected result"
  where
    extractPair (OArray [k, v]) = pure (oValueToText k, v)
    extractPair (OObject kvs)
        | Just k <- lookupO "key" kvs
        , Just v <- lookupO "value" kvs =
            pure (oValueToText k, v)
    extractPair (OObject kvs) = case kvs of
        [(k, v)] -> pure (k, v)
        _multipleKeys -> tagSyntaxError meta (Just "!$mapListToHash") "!$mapListToHash: object item must have exactly one key"
        ...
        OObject kvs -> do
            pairs <-
                traverse
                    ...
                    kvs
            pure $ OObject pairs
        _nonObject -> typeMismatchError meta "object" itemsVal
...
resolveFromPairs ctx meta (FromPairsTag sourceAst) = do
    ...
    case sourceVal of
        OArray arr -> do
            pairs <- traverse extractPair arr
            pure $ OObject pairs
        _nonArray -> typeMismatchError meta "sequence" sourceVal
  where
    extractPair (OArray [k, v]) = pure (oValueToText k, v)
...
resolveParseYaml ctx meta (ParseYamlTag strAst) = do
    ...
    case val of
        ...
        _nonString -> typeMismatchError meta "string" val
