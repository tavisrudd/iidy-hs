...
import Amazonka.CloudFormation.DescribeStackResources qualified as DSR
import Amazonka.CloudFormation.DescribeStacks qualified as DS
import Amazonka.CloudFormation.ListExports qualified as LE
                        ...
                        Nothing -> do
                            let km =
                                    List.foldl'
                                        ( \acc r ->
                                            KM.insert
                                                (Key.fromText r.logicalResourceId)
                                                (resourceToValue r)
                                                acc
                                        )
                                        KM.empty
...
withStack ::
    Amazonka.Env ->
    -- | original location (for error messages)
    Text ->
    -- | stack name
    Text ->
...
mkImportData location rawData doc =
    ImportData
        { idType = ImportCfn
        , idLocation = location
        , idRawData = rawData
