...
import Iidy.Yaml.CustomResources.RefRewriting (collectGlobalRefs, rewriteRefs)
...
expandCustomResource ::
    -- | Resource instance name (e.g., "OrderEvents")
    Text ->
    -- | Resource definition (Properties, Overrides, etc.)
    OValue ->
    -- | Template definition
    TemplateInfo ->
    -- | Re-parser: given params, re-parse and resolve template body
    (Map Text OValue -> Text -> Either Text OValue) ->
    -- | Additional global refs (parent resource names, not rewritten)
    Set Text ->
...
prefixAndRewriteSection prefix globals = \case
    OObject kvs ->
        OObject
            [ (key', stripGlobal (rewriteRefs prefix globals v))
            | (k, v) <- kvs
