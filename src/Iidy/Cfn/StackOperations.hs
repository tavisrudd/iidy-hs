...
import Amazonka.CloudFormation.DescribeStackEvents qualified as DEvents
import Amazonka.CloudFormation.DescribeStackResources qualified as DRes
...
collectStackContentsWithStack ctx sName mStack = do
  ...
  where
    resReq =
        DRes.newDescribeStackResources
...
data PollConfig = PollConfig
    ...
    -- ^ operation start time (for duration calc)
    , pcWaitForStatusChange :: !Bool
    -- ^ wait for new events before checking terminal (watch-stack)
    ...
    , pcOnPollTick :: IO ()
    -- ^ called each poll cycle (for spinner)
    }
...
pollForCompletion ::
    CfnContext ->
    -- | stack ID (use ID not name for deletes)
    Text ->
    -- | terminal statuses
    [StackStatus] ->
    PollConfig ->
...
pollForCompletionWith ::
    -- | event fetcher
    IO [CF.StackEvent] ->
    -- | stack ID (for isStackEvent check)
    Text ->
    -- | terminal statuses
    [StackStatus] ->
    PollConfig ->
    ...
    let go :: Set.Set Text -> IO PollResult
        go lastEventSet = do
            ...
            case pcInactivityTimeoutSecs config of
                Just timeout
                    | timeout > 0 && null newEvents && inactivityElapsed > timeout
