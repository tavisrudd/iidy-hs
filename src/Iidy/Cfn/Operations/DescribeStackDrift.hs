...
import Amazonka.CloudFormation.DescribeStackDriftDetectionStatus qualified as DSDDS
import Amazonka.CloudFormation.DescribeStackResourceDrifts (DescribeStackResourceDrifts (nextToken))
import Amazonka.CloudFormation.DescribeStackResourceDrifts qualified as DSRD
import Amazonka.CloudFormation.DetectStackDrift qualified as DSD
...
describeStackDrift ::
    ...
    -- | Stack name
    Text ->
    -- | Drift cache seconds (default 300)
    Int ->
    -- | emit callback
    (OutputData -> IO ()) ->
    IO (Either Text ())
                ...
                unless completed $ do
                    ...
                    emit
                        ( OdStatusUpdate
                            StatusUpdate
                                { suMessage =
                                    "Drift detection timed out after "
