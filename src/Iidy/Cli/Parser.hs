...
import Paths_iidy_hs qualified as Paths
...
versionOption =
    infoOption
        ("iidy-hs " <> showVersion Paths.version)
        ( long "version"
            <> short 'V'
            <> help "Print version"
        )
...
statusCodesDoc :: Doc
statusCodesDoc =
    pretty ("Status Codes:" :: String)
        <> hardline
        <> pretty ("  Success (0)       Command successfully completed" :: String)
        <> hardline
        <> pretty ("  Error (1)         An error was encountered while executing command" :: String)
        <> hardline
    ...
    subparser
        ( command
            ...
            <> command
                "explain"
                ( info
                    (CmdExplain <$> many (argument textReader (metavar "CODE...")) <**> helper)
...
hiddenCommands =
    subparser
        ( command
            "get-stack-instances"
            ( info
                (CmdGetStackInstances <$> getStackInstancesArgsParser <**> helper)
...
paramCommandsParser =
    subparser
        ( command
            "set"
            ( info
                (ParamSet <$> paramSetArgsParser <**> helper)
                (progDesc "set a parameter value")
            )
            <> command
                "review"
                ( info
                    (ParamReview <$> paramPathArgParser <**> helper)
                    (progDesc "review a pending change")
                )
            <> command
                "get"
                ( info
                    (ParamGet <$> paramGetArgsParser <**> helper)
                    (progDesc "get a parameter value")
                )
            <> command
                "get-by-path"
                ( info
                    (ParamGetByPath <$> paramGetByPathArgsParser <**> helper)
                    (progDesc "get parameters by path")
                )
            <> command
                "get-history"
                ( info
                    (ParamGetHistory <$> paramGetArgsParser <**> helper)
...
approvalCommandsParser =
    subparser
        ( command
            "request"
            ( info
                (ApprovalRequest <$> approvalRequestArgsParser <**> helper)
                (progDesc "request template approval")
            )
            <> command
                "review"
                ( info
                    (ApprovalReview <$> approvalReviewArgsParser <**> helper)
