...
import Iidy.Yaml.Imports.Loaders.Cfn (CfnField (..), parseCfnLocation)
import Iidy.Yaml.Imports.Loaders.S3 (parseS3Uri)
import Iidy.Yaml.Imports.Loaders.Ssm (buildResultObject, parseSsmLocation, parseSsmPathLocation, stripPathPrefix)
...
ssmPathResultTests =
    ...
    , testCase "buildResultObject with >10 params (multi-page simulation)" $ do
        ...
        let basePath = "/app/config"
            params =
                [ ( "/app/config/param-" <> T.pack (show i)
                  , "value-" <> T.pack (show i)
                  )
                | i <- [1 .. 15 :: Int]
                ]
