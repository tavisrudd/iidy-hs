...
import Amazonka.CloudFormation.Types.ChangeSetSummary qualified as CSS
import Amazonka.CloudFormation.Types.Output qualified as Out
import Amazonka.CloudFormation.Types.StackResource qualified as SR
import Amazonka.Data.Time (Time (..))
import Iidy.Cfn.StackOperations (convertChangeSetSummary, convertOutput, convertResource)
...
stackOpsConverterTests =
    [ testGroup
        ...
        [ testCase "converts all fields from CF.StackResource" $ do
            let cfRes =
                    (SR.newStackResource "MyBucket" "AWS::S3::Bucket" epoch CF.ResourceStatus_CREATE_COMPLETE)
