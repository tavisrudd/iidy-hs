...
import Iidy.Yaml.Engine (PreprocessError (..), PreprocessResult (..), preprocessYaml)
...
alwaysSucceedLoader _loc _base =
    pure $
        Right $
            ImportData
                { idType = ImportFile
        ...
        [ testCase "recursively preprocesses imported doc with nested $imports" $ do
            ...
            case result of
                ...
                Right _ -> do
                    ...
                    assertBool
                        "inner.yaml should have been loaded via recursive preprocessing"
