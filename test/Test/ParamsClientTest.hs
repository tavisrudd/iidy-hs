...
paramOutputConversionTests =
    [ testCase "extracts all fields from Parameter" $ do
        let p =
                mkParamFull "/myapp/db-pass" SSMPT.ParameterType_SecureString "secret" 3
...
paramOutputJsonTests =
    ...
    , testCase "Tags included when present" $ do
        let out =
                (paramOutputFromParameter (mkParam "/test" "val"))
...
paramHistoryJsonTests =
    ...
    , testCase "Tags included when present" $ do
        let out =
                (paramHistoryOutputFromHistory mkHistoryFull)
...
assertNotContains needle haystack =
    assertBool
        ("Expected NOT to find " <> show needle <> " in:\n" <> T.unpack haystack)
