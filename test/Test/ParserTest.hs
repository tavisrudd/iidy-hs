...
import Iidy.Yaml.Ast (YamlAst(..), SrcMeta(..), astMeta)
...
spanTests =
  ...
  , testCase "mapping: end position extends past last value" $ do
      ...
      case ast of
        AstMapping _ m -> do
          ...
          assertBool "mapping smEnd offset should be past smStart"
  ...
  , testCase "mapping: multi-line end position on last value line" $ do
      ...
      case ast of
        AstMapping _ m -> do
          ...
          assertBool "mapping smEnd line should be past smStart line"
  ...
  , testCase "sequence: end position extends past last element" $ do
      ...
      case ast of
        AstSequence _ m -> do
          ...
          assertBool "sequence smEnd line should be past smStart line"
            (posLine (smEnd m) > posLine (smStart m))
  ...
  , testCase "nested mapping: outer span extends to inner content" $ do
      ...
      case ast of
        AstMapping _ m -> do
          assertBool "nested mapping smEnd should be past smStart"
            (posOffset (smEnd m) > posOffset (smStart m))
        other -> assertFailure ("expected AstMapping, got: " <> show other)
