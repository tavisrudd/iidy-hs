...
| 1b | Shallow module: CfnStatus (lookup table, no transitions)        | RESOLVED            | `StackStatus` ADT with 22 constructors, pattern-match predicates, conversion functions (4A). No longer a shallow lookup table — module now provides a proper type with real logic. |
| 2a | Information leakage: CfnContext(..) exports all fields          | OPEN                | No change to CfnContext export list or abstraction boundary.                                                      |
| 2b | Information leakage: StackArgs(..) — all 21 fields exposed      | PARTIALLY ADDRESSED | OnFailure/Capability ADTs; `saStackName` non-optional (1E); unknown-key validation with did-you-mean (1D). StackArgs still exports all fields; per-operation config types not introduced. |
...
| 4b | Tactical: error handling — 5 patterns, no unified algebra        | PARTIALLY ADDRESSED | TemplateLoader `fail` → `Either Text` (6 sites). `try @SomeException` narrowed at 13 sites (3A). GlobalConfig catches `Amazonka.Error` (1G). Reduced from 5 to 3 patterns. `IidyError` sum type not introduced. |
...
| 6b | Interface smell: emptyStackArgs with 21 Nothing fields           | PARTIALLY ADDRESSED | OnFailure/Capability ADTs, non-optional stackName (1E), unknown-key validation (1D). Still 20 Maybe fields with no per-operation distinction. |
| 7  | Strategic redesign: operation-specific argument types             | PARTIALLY ADDRESSED | OnFailure/Capability ADTs, non-optional stackName, unknown-key validation, getStrListValidated. Full CreateStackConfig / DeleteStackConfig not yet introduced. |
