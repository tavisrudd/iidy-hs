...
|----|-----------------------------------------------------------|----------------------|--------------------------------------------------------------------------------------------------------------------|
| 1  | StackArgs contradictory field combinations                | PARTIALLY ADDRESSED  | `saOnFailure` → `Maybe OnFailure` ADT; `saCapabilities` → `Maybe [Capability]` ADT; `saStackName` → non-optional `!Text` (1E); unknown-key validation with did-you-mean (1D); `getStrListValidated` rejects non-strings (1C). Per-operation config types remain OPEN (4C). |
| 2  | Stack statuses stringly typed                             | RESOLVED             | `StackStatus` ADT with 22 constructors replaces `Text` throughout (4A). Amazonka types converted at boundary, predicates use pattern matching. |
...
| 6  | Error types as Text everywhere                            | PARTIALLY ADDRESSED  | TemplateLoader `fail` → `Either Text` (6 sites). `ConfirmResult` ADT replaces Bool (3C). `try @SomeException` narrowed at 13 sites (3A). `Either Text` → `Either IidyError` across CFN layer remains OPEN. |
