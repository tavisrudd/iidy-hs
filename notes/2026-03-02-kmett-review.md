...
| 3   | The monad stack that isn't (5 error patterns)  | PARTIALLY ADDRESSED | TemplateLoader `fail` → `Either Text`. `try @SomeException` narrowed to specific types at 13 AWS boundaries (3A). GlobalConfig catches `Amazonka.Error` not `SomeException` (1G). Remaining: `catch` + stderr, `throwIO`. Full `ExceptT`/`MonadError` unification deferred. |
